...
class Kamyon:
  ...
  def secim_yap(self,cluster,kamyonSayisi,dolulukList,ziyaretList,enYakinKontrol,alfa,beta,omega,nodeList,mesafeMatrix,ziyaretMatrix):
    probList=list()
    indexList=list()
    zMatrix=[]
    ...
    for i in range(1,len(zMatrix)):
        ix = zMatrix[i]
        if nodeList[ix].id != self.mevcutKonum.value:
            dist = mesafeMatrix[self.mevcutKonum.value][nodeList[ix].id]
            enYakin.append([dist,nodeList[ix].id])
    #print(enYakin)
    ...
    for i in range(len(ds)):
        ...
        prob = pow(((dist)),alfa) * pow((1/ziyaretCnt),omega)
